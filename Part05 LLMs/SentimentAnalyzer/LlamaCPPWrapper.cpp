...
bool gpt_params_parse(int argc, char** argv, gpt_params& params) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-s" || arg == "--seed") {
            ...
            params.n_batch = min(512, params.n_batch);
        }
        else if (arg == "--keep") {
            if (++i >= argc) {
                invalid_param = true;
...
LlamaCPPWrapper::LlamaCPPWrapper(std::string pathToModel)
{
    std::cout << "Initialising llama cpp\r\n";
    params.prompt = "Hello my name is";
    params.model = pathToModel;
    params.n_ctx = 2048;
    params.repeat_penalty = 1.1;
    params.antiprompt.push_back("### Input:");
    params.instruct = true;
    params.interactive = true;
    params.prompt_cache_all = true;
    params.temp = 0.40;
...
std::string LlamaCPPWrapper::Evaluate(std::string prompt,int nPast)
{
   
    bool bAntiPromptReached = false;
   ...
   std::vector<llama_token> tokens_list;
    ...
    while (llama_get_kv_cache_token_count(ctx) < max_context_size)
    {
        ...
        auto n_past = llama_get_kv_cache_token_count(ctx);
        ...
        std::string sToken(llama_token_to_str(ctx, new_token_id));

        sRet += sToken;

        auto antiPrompts = params.antiprompt.size();

        for (auto i = 0; i < params.antiprompt.size();i++)
        {
            auto antiPrompt = params.antiprompt[i];

            if (sRet.size() >= antiPrompt.size())
            {
                if (sRet.substr(sRet.size() - antiPrompt.size()) == antiPrompt)
                {
                    bAntiPromptReached = true;
                }
            }
...
int LlamaCPPWrapper::GetCacheDepth()
{
    return llama_get_kv_cache_token_count(ctx);;
}

int LlamaCPPWrapper::GetStateSize()
{
    return llama_get_state_size(ctx);;
}

uint8_t* LlamaCPPWrapper::SaveState()
{
    const size_t cacheSize = llama_get_state_size(ctx);

    uint8_t* statePointer = new uint8_t[cacheSize];

    llama_copy_state_data (ctx, statePointer);

    return statePointer;
}

void LlamaCPPWrapper::LoadState(uint8_t* state)
{
   
    llama_set_state_data(ctx, state);
    
}

void LlamaCPPWrapper::ReleaseState(uint8_t* state)
{
    delete state;
}
